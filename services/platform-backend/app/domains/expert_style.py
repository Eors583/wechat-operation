...
SECOND_PERSON_RE = re.compile(r"你们|你|\byou\b", re.IGNORECASE)
FIRST_PERSON_PLURAL_RE = re.compile(r"我们|\bwe\b", re.IGNORECASE)
...
def banned_phrase_audit(text: str) -> dict:
    ...
    for phrase in BANNED_PHRASES:
        cnt = len(re.findall(r"(?<![a-z])" + re.escape(phrase) + r"(?![a-z])", lowered))
        if cnt:
...
def build_profile(texts: list[str]) -> dict:
    ...
    avg_para_sentences = (
        round(mean(paragraph_sentence_counts), 2) if paragraph_sentence_counts else 0
    )
    question_density = round((question_count / max(len(sentences), 1)) * 100, 2)
    ...
    profile = {
        ...
        "rhythm": {
            ...
            "question_density_per_100_sentences": question_density,
        },
        "voice": {
