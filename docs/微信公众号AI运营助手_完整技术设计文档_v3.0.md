...
新增字段通过 Alembic 扩展迁移上线。后台每 60 秒扫描有效用户已连接且未删除的公众号，
当 `writing_profile` 是空 JSON 对象时，复用原有学习任务和 Outbox 自动补齐历史账号，
不需要用户重新授权。每批最多 50 个，账号行锁跳过并发处理中记录，并排除已有进行中任务
或一小时内已创建任务的账号；入队时字段即变为 queued，因此后续扫描不会重复入队。
已有画像以及非空的排队、处理中、失败记录不自动重建，失败仍走现有管理端重试入口。
