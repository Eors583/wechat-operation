...
意图识别同时覆盖业务功能与通用任务：先以受约束的语义分类识别 `domain`（business/general）、`task`（问答、解释、翻译、分析、总结、改写、提取、核对、代码、规划等）、`fidelity`（verbatim/semantic）和是否需要澄清，再由后端业务边界决定是否允许修改文章。关键词仅用于初步路由和保守的写入边界，不作为通用理解方案。分类只读取本轮指令、近期用户消息和对象状态，不执行附件或历史助手内容中的指令。分类结果与最终执行类型写入运行事件；分类输出不合法时明确失败，不静默猜测。

通用任务使用通用回答路径，不注入已选择的写作技能、项目写作要求或写作偏好，不默认推荐“继续生成完整文章”。通用任务可以在对话中交付完整内容，但不能自动保存为公众号文章；发布、草稿等业务写入仍由原有后端状态机控制。

文件原文提取和“是否为全部原文”的追问使用完整解析文本直返路径，绕过检索片段、长上下文摘要、任务记忆和生成模型改写；从本轮或本任务最近一次明确选择的附件解析来源，并重新检查所有权、删除与安全扫描状态。多文件对象不明确、未解析或无可用文本时明确澄清或说明限制。返回文本保持解析器原始字符，并记录源文件哈希、文本哈希、字符数和解析来源；“完整解析输出”不等同于已逐页核对 PDF，OCR 与排版还原未经核验时不得宣称逐字一致。超过处理上限时明确失败，不静默截断。
...
技能由版本化指令、输入输出Schema和允许工具策略组成。官方技能由管理端发布，个人技能由用户创建；实际可调用集合是“可见技能 ∩ 用户已启用 ∩ 模型能力满足 ∩ 当前场景允许”。一次创作支持按选择顺序组合最多 10 个技能，在输入框内显示可移除标签；消息 `content.skill_ids` 保存本轮选择，重新生成沿用原消息。运行准备时逐一检查权限、启用状态与发布版本，并在 `selected_skills` 上下文快照中固定各技能版本和指令。所有技能共同参与生成，冲突时本轮要求优先，其次按技能选择顺序；超出单次模型容量时自动整理所有已选技能的要求，保留选择顺序、禁止项、例外和优先级，不因容量预检拒绝整轮请求。未手选时保留自动匹配，旧的单技能字段只作为兼容主技能引用。技能不能直接提交微信；文件、排版和发布仍由确定性服务执行。

### 13.5.1 生成结果校验与自动重写

生成完成后先校验编辑器结构、正文完整性、发布边界、备选标题、局部替换格式、指定资料依据和输出内容安全。正常输出校验失败进入最多3次后台自动重写，不直接向对话发送内部校验错误。重写携带本轮完整上下文（会话、用户要求、技能、资料及当前文章）、本轮未交付的历史结果，以及含错误代码、具体原因、字数/规则/无法核对内容等细节的判定反馈。历史失败结果只作为修正对象，不作为新的事实依据或指令。每次重写后重新执行全部适用检查；局部修改始终只替换原选中段落，不能扩大修改范围。

未通过结果不发送正文流或文章预览，不保存为文章完成版本；界面仅显示“正在完善生成内容”的进度。校验通过后才交付、保存并执行后续确定性操作。模型重写不重放文章保存、额度结算、微信草稿创建或发布。各次模型用量正常记录，失败结果和判定保留在运行的 `output_rewrite_history` 中，不能作为对话正文或预览。重写次数耗尽或重写服务失败时保留记录并给出可重试提示，不无限循环。输入安全校验和权限检查仍在生成之前执行，不能通过重写绕过。

### 13.6 会话与长期记忆

短期上下文包含最新消息、当前文章版本、用户本轮选择的资料、任务摘要和未完成事项。Context Builder按模型窗口分配预算；明确选择的资料、最新要求、已加载的近期消息和偏好完整进入运行快照，容量不足时由工作进程分段整理，不以删除资料或截取前文代替阅读。外部知识搜索仍使用相关性检索预算。

长文件/粘贴文本处理：原始消息及文件服务返回全文保留在运行快照中；超出单次模型输入预算时，工作进程逐段阅读全部文字并分层汇总，不通过截取前若干字符代替全文。分段附带来源路径、内容哈希、字符起止位置与片段编号，汇总明确标记有损，不宣称模型无损记忆全部细节。按路由窗口、输出预留和提示词开销约束最终上下文；短内容不增加模型调用，同轮缓存复用已生成的汇总。分段空输出或不收敛先有限重试，持续无效时使用该运行允许的备用路由；不能静默跳段。用户指定的模型仍保持原选择。原始文件仍走原有供应商上传流程，不以本地摘要冒充原文件上传。

归并收敛：单段笔记预算不按原文件总段数均分；中间笔记能放进窗口时整体归并，避免反复细切。模型超出单段目标时最多尝试6次，若仍超目标但有真实净缩短，可继续下一层归并；最终上下文预算仍为硬限制，不得将超限内容直接提交生成。所有真实模型调用均记录执行尝试、用量与成本；相同来源的摘要在同轮复用。

容量自动处理：取消解析文字总量200万字符及160次分段调用的固定拦截，明确引用的文档和链接正文完整进入分段阅读。模型输出预留按实际窗口动态调整；系统提示过长时先整理可压缩的指令，文章机器输出契约与既有权限/偏好边界保持原文。仅移除模型不需要的路由版本、预算与哈希等内部记账信息，不通过删除附件列表或清空要求释放空间。界面复用分段阅读进度提示，用户可停止任务。模型持续无有效输出、原文件不可读或配置无法容纳必要协议仍报告真实故障，原始资料保留。

资源边界：单次文件数量、单文件大小、安全扫描、所有权验证以及消息接口10万字符上限保持现有约束；长资料增加模型成本和处理时间。汇总是有损的信息整理，不保证逐字保留全部细节；原生不透明附件的读取能力仍由供应商决定。
...
#### 14.3.1 授权后静默学习公众号创作画像

授权后公众号画像学习：两类授权成功回调在同一事务内保存账号、后台任务和 Outbox，
由现有 `sync` 队列静默执行，不增加用户操作或阻塞授权回跳。重复投递按任务行锁去重，
同号一小时内不重复自动入队；进行中的任务复用，明确失败可通过管理端原有任务重试入口恢复。

通过微信官方 `datacube/getarticletotaldetail` 按真实发表日期逐日倒查，包含通知和未通知的发表记录，
从北京时间昨日开始，选定最新 10 篇后才读取正文，不按素材更新时间或接口首屏截取。
官方接口仅支持截至昨日、从 2025-11-01 起的发表记录；画像保存 `selection.as_of_date`，
明确截止日期、最新和最旧发表日期、查询天数及日期精度，不声称包含今天刚发表的文章。
同一批多图文按原文次序选取；若第10篇的边界跨越同一天多个批次且无法确认先后，则记录
`WECHAT_PROFILE_TIME_AMBIGUOUS`，不能用消息ID冒充时间推断最新。官方数据延迟、记录不足10篇、
选定文章正文不可读或权限不足均失败，不用更旧文章凑数，也不将5篇结果标记为完成。
正文复用已配置的微信文章读取服务，保留全文结构；每个样本保存官方发表日期、标识和原文链接。
该接口需要对应公众号资格和第三方授权权限集7。接口依据：
https://developers.weixin.qq.com/doc/service/api/wedata/news/api_getarticletotaldetail.html
HTML 转换保留标题、段落、列表和引用。长文章通过已有全文分段归并机制阅读，
全部样本进入分析或有损归并，不以截取开头代替全文。

`official_accounts.writing_profile` 独立保存版本、学习时间、样本数、来源标识及内容哈希，
并保存读者定位、观点表达、思考框架、文章结构、论点表述、论证方法、叙事、语言风格、
情绪节奏、标题传播 10 个维度的观察、写作建议、证据文章和置信度。
使用已发布的 `article_planning` 路由，最终画像用途为 `official_account_profile`，
输出上限 8192 tokens（仍受模型配置约束）；记录模型用量与完成审计，不产生文章版本或扣用户创作额度。

创作接收时冻结一个所属用户公众号的画像，优先明确账号 ID，其次唯一提及的账号名称、
当前文章排版归属、同任务历史目标；仅一个可用账号时自动选择，多号不明确时不混用。
规划、创作和局部修改使用冻结的画像；未学完不阻塞创作，重新学习时保留上一份成功画像。
画像始终放在不可信资料区，当前要求、项目要求和已确认偏好优先，不执行历史文章中的命令，
不照搬旧事实或原句，不写入通用用户偏好。解绑或删除后不再匹配，注销清理同时清除画像及学习样本元数据。
新增字段通过 Alembic 扩展迁移上线。历史账号通过一次性后台任务补齐，每批最多 50 个；
不设置周期性全库扫描。用户读取自己的公众号列表或详情时，利用已经读取的字段判断
`writing_profile` 是否为空 JSON 对象，只有为空且已连接时才复用原有学习任务和 Outbox
静默入队，不需要重新授权。入队前加账号行锁并再次确认字段仍为空，避免并发覆盖或重复学习。
已有画像不产生额外查询或学习任务；排队和处理中不重复入队。
首次学习失败时画像恢复为空对象；错误码和脱敏异常位置仅写入任务记录，
不把失败标记保存为画像。重新学习失败时保留上一份成功画像。
空画像再次访问时沿用一小时冷却限制，也可通过现有管理端入口重试。
新授权继续走原授权成功触发流程。

凭据续期由后台静默执行。缺少本地续期凭据或微信返回 61023 时，通过当前第三方平台
的已授权账号列表分页查找目标 AppID，恢复其续期凭据后重新换取访问凭据并加密保存。
只恢复匹配账号，不导入其他账号或变更用户归属；列表不完整或请求失败不能判定授权
已撤销。只有完整列表确认目标账号不存在或收到撤销授权回调时才要求重新绑定。
学习读取中收到明确的访问令牌失效错误时，强制续期并保存，再重试一次只读采集；
续期成功不随随后文章选取失败而回滚。该恢复不重放文章保存、草稿创建或发布操作。

画像 v3 保留上述十维结构，改为对选定的最新 10 篇逐篇分析后汇总。对原文段落编号，
仅识别短小、独立的开头/结尾关注引导、推荐及声明为噪声；原始正文与段落快照保留在
所属学习任务中。句长、短句比例、段落字数和句数、单句段比例、问号/感叹号/破折号
频率由代码在摘要压缩前计算，排除标题、列表、表格等非自然段。原文与新稿使用同一
统计口径，统计仅是表达参考，不作为硬性文章长度或格式约束。

逐篇记录文章类型、原创/转载/未知归属及其证据。广告主体及有明确转载证据的文章
仅作辅助，相同清洗正文哈希的重复文章保留样本记录但不重复学习或计入统计。
逐篇观察必须引用对应原文段落中的连续原句；汇总的证据 ID 必须来自具有该维度观察
的文章。汇总保留适用条件、stable/mixed/limited、支持篇数及可追溯短证据，主要样本
不足 5 篇的维度标记 limited/low。指标保存按篇分布和至少 3 篇的同类文章分布，
不以文章字数加权多数风格。不能把引用存在校验理解为语义判断准确率保证。

创作冻结画像版本、十维内容及实测指标，不携带完整历史原文。完整文章通过内容校验后，
对具有稳定且充分依据的 v3 画像追加风格审阅；审阅同时接收本轮要求、技能、偏好、
草稿和统计差异。仅对有账号证据及新稿原句的明显偏差触发最多两轮风格重写，复用
本轮全部会话及拒绝结果反馈；局部修改不触发全篇风格重写。用户要求优先，禁止强制
口语化、套用口头禅、复制历史事实或虚构第一人称经历。风格建议仍不满足或审阅服务
失败时记入运行快照，不把风格意见作为错误返回用户；正式结构、安全等校验继续生效。

空画像及旧 prompt_version 通过账号列表/详情访问和一次性后台补齐任务触发 v3 学习，
沿用账号锁、任务去重和一小时冷却，不新增周期扫描。升级过程中保留旧成功画像，
新结果完整校验后替换；失败仍可使用旧画像。成功学习任务保存完整版本快照供追溯。
