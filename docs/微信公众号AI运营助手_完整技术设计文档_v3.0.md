...
通过微信官方 `freepublish/batchget` 读取可访问的近期已发表文章，多图文按单篇展开，
按接口返回的更新时间排序，去重并跳过删除、图片或装饰空正文，取最多 20 篇。
不足时按实际样本学习；没有正文、权限不足、分页异常或超出容量时记录失败，不生成虚假画像。
该接口需要对应公众号资格和第三方授权权限集 7；更新时间不冒充发表时间。
