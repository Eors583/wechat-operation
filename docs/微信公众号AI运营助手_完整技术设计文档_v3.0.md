...
技能由版本化指令、输入输出Schema和允许工具策略组成。官方技能由管理端发布，个人技能由用户创建；实际可调用集合是“可见技能 ∩ 用户已启用 ∩ 模型能力满足 ∩ 当前场景允许”。一次创作支持按选择顺序组合最多 10 个技能，在输入框内显示可移除标签；消息 `content.skill_ids` 保存本轮选择，重新生成沿用原消息。运行准备时逐一检查权限、启用状态与发布版本，并在 `selected_skills` 上下文快照中固定各技能版本和指令。所有技能共同参与生成，冲突时本轮要求优先，其次按技能选择顺序；超出模型容量时明确提示，不静默丢弃技能。未手选时保留自动匹配，旧的单技能字段只作为兼容主技能引用。技能不能直接提交微信；文件、排版和发布仍由确定性服务执行。
