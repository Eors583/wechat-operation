...
从北京时间昨日开始，选定最新 20 篇后才读取正文，不按素材更新时间或接口首屏截取。
官方接口仅支持截至昨日、从 2025-11-01 起的发表记录；画像保存 `selection.as_of_date`，
明确截止日期、最新和最旧发表日期、查询天数及日期精度，不声称包含今天刚发表的文章。
同一批多图文按原文次序选取；若第20篇的边界跨越同一天多个批次且无法确认先后，则记录
`WECHAT_PROFILE_TIME_AMBIGUOUS`，不能用消息ID冒充时间推断最新。官方数据延迟、记录不足20篇、
...
画像 v3 保留上述十维结构，改为对选定的最新 20 篇逐篇分析后汇总。对原文段落编号，
