...
const taskStatus = (value: unknown): TaskStatus => {
  ...
  if (['processing', 'generating', 'submitting', 'reconciling'].includes(status)) return 'running'
  return ['failed', 'unknown', 'completed', 'cancelled'].includes(status)
    ? (status as TaskStatus)
    : 'queued'
}
const userStatus = (value: unknown): UserStatus => (value === 'disabled' ? 'disabled' : 'active')
...
async function loadPublishedAiSettings(): Promise<JsonRecord> {
  ...
  const current =
    [...response.items].filter((item) => item.status === 'published').sort(byVersion)[0] ??
    [...response.items].sort(byVersion)[0]
...
export const adminRepository = {
  async saveModelConfiguration(
    ...
    if (exists) {
      await adminOpenApiData(
        adminOpenApi.PATCH('/admin-api/v1/model-configurations/{configuration_id}', {
  ...
  async testModelConfigurationBeforeSave(
    model: ModelConfiguration,
    apiKey: string,
  ): Promise<{ passed: boolean; message: string; validation_token: string | null }> {
  ...
  async testModelConfiguration(
    ...
    return adminOpenApiData(
      adminOpenApi.POST('/admin-api/v1/model-configurations/{configuration_id}/test', {
  ...
  async deleteModelConfiguration(configurationId: string): Promise<void> {
    await adminOpenApiData(
      adminOpenApi.DELETE('/admin-api/v1/model-configurations/{configuration_id}', {
        params: { path: { configuration_id: configurationId } },
      }),
    )
  },
  ...
  async setModelConfigurationStatus(
    ...
    await adminOpenApiData(
      adminOpenApi.PATCH('/admin-api/v1/model-configurations/{configuration_id}/status', {
        params: { path: { configuration_id: configurationId } },
        body: { status },
      }),
    )
  ...
  async testRoute(routeId: string): Promise<{ passed: boolean; message: string }> {
    return adminOpenApiData(
  ...
  async recordUserDiagnostic(userId: string, reason: string): Promise<{ message: string }> {
    return adminOpenApiData(
  ...
  async dashboard(): Promise<{ summary: DashboardSummary; jobs: JobRecord[] }> {
    ...
    return {
      summary: {
        ...
        ai_success_rate: aiTotal ? Math.round(((aiTotal - aiFailed) / aiTotal) * 1000) / 10 : 100,
  ...
  async jobs(): Promise<JobRecord[]> {
    const response = await adminOpenApiData(
      adminOpenApi.GET('/admin-api/v1/jobs', { params: { query: { limit: 500 } } }),
    )
    return response.items.map((item): JobRecord => {
      ...
      return {
        ...
        owner_display_name: item.owner_id ? `用户 ${item.owner_id.slice(0, 8)}` : '系统',
        ...
        model_or_account: text(frozen.model_id, text(frozen.official_account_id, item.queue)),
        ...
        ...(text(frozen.external_id) ? { external_id: text(frozen.external_id) } : {}),
  ...
  async modelConfigurations(): Promise<ModelConfiguration[]> {
    const response = await adminOpenApiData(adminOpenApi.GET('/admin-api/v1/model-configurations'))
    return response.items.map((item) => {
      ...
      return {
        ...
        model_type: ['chat', 'embedding', 'rerank', 'vision'].includes(item.model_type)
  ...
  async prompts(): Promise<PromptVersion[]> {
    const bundles = await adminOpenApiData(adminOpenApi.GET('/admin-api/v1/prompt-bundles'))
    const details = await Promise.all(
      bundles.items.map((bundle) =>
        adminOpenApiData(
          adminOpenApi.GET('/admin-api/v1/prompt-bundles/{bundle_id}/versions', {
  ...
  async savePrompt(prompt: PromptVersion, exists: boolean): Promise<string> {
    const bundles = await adminOpenApiData(adminOpenApi.GET('/admin-api/v1/prompt-bundles'))
    const bundle = bundles.items.find((item) => item.code === prompt.bundle_code)
  ...
  async skills(): Promise<OfficialSkill[]> {
    const response = await adminOpenApiData(adminOpenApi.GET('/admin-api/v1/official-skills'))
    const versions = await Promise.all(
      response.items.map((item) =>
        adminOpenApiData(
          adminOpenApi.GET('/admin-api/v1/official-skills/{skill_id}/versions', {
    ...
    return response.items.map((item, index) => {
      ...
      return {
        ...
        scenarios: [item.category],
        instructions: version?.instructions ?? '',
        version: version?.version_no ?? item.current_version_no,
        sort_order: item.sort_order,
        status: configStatus(
          item.status === 'disabled' ? 'disabled' : (version?.status ?? item.status),
        ),
        enabled_users: 0,
        updated_at: item.updated_at,
      }
      ...
      await adminOpenApiData(
        adminOpenApi.PATCH('/admin-api/v1/official-skills/{skill_id}', {
          ...
          body: {
            ...
            category: skill.scenarios[0] ?? 'content',
  ...
  async users(): Promise<UserRecord[]> {
    const list = await adminOpenApiData(
      adminOpenApi.GET('/admin-api/v1/users', { params: { query: { limit: 200 } } }),
    )
    const details = await Promise.all(
      list.items.map((item) =>
        adminOpenApiData(
    ...
    return details.map(({ user, quota, limits, official_accounts, failed_jobs }) => ({
      id: user.id,
  ...
  async wechatConfigs(): Promise<WechatPlatformConfig[]> {
    ...
    return response.items.map((item) => ({
      ...
      affected_capabilities:
        item.status === 'published' && item.last_ticket_at && item.last_token_refresh_at
          ? []
          : ['新增公众号绑定', '公众号草稿与发布'],
    }))
  },
  ...
  async saveWechatConfig(
    ...
    secretRefs: { appSecret?: string; messageToken?: string; encodingAesKey?: string },
  ): Promise<void> {
    await adminOpenApiData(
  ...
  async testWechatConfig(): Promise<{ passed: boolean; message: string }> {
    return adminOpenApiData(
      adminOpenApi.POST('/admin-api/v1/wechat-platform-configs/{environment}/test', {
        params: { path: { environment: 'production' } },
      }),
    )
  },
  ...
  async publishWechatConfig(): Promise<void> {
    await adminOpenApiData(
      adminOpenApi.POST('/admin-api/v1/wechat-platform-configs/{environment}/publish', {
        params: { path: { environment: 'production' } },
      }),
    )
  },
  ...
  async officialAccounts(): Promise<OfficialAccount[]> {
    const [response, users] = await Promise.all([
      adminOpenApiData(
        adminOpenApi.GET('/admin-api/v1/official-accounts', { params: { query: { limit: 500 } } }),
      ),
      adminOpenApiData(
        adminOpenApi.GET('/admin-api/v1/users', { params: { query: { limit: 200 } } }),
      ),
    ])
    const names = new Map(users.items.map((item) => [item.id, item.display_name]))
    return response.items.map((item) => {
      ...
      return {
        ...
        owner_display_name: names.get(item.owner_id) ?? `用户 ${item.owner_id.slice(0, 8)}`,
        status,
  ...
  async refreshOfficialAccount(accountId: string, reason: string): Promise<void> {
    await adminOpenApiData(
      adminOpenApi.POST('/admin-api/v1/official-accounts/{account_id}/refresh', {
  ...
  async markOfficialAccountReconnect(accountId: string, reason: string): Promise<void> {
    await adminOpenApiData(
      adminOpenApi.POST('/admin-api/v1/official-accounts/{account_id}/mark-reconnect', {
        params: { path: { account_id: accountId } },
        body: { reason },
      }),
    )
  ...
  async systemSettings(): Promise<SystemSettings> {
    const response = await adminOpenApiData(adminOpenApi.GET('/admin-api/v1/system-settings'))
    ...
    const merged = Object.assign({}, ...latest.map((item) => record(item.values)))
    const newest = [...latest].sort((a, b) => b.created_at.localeCompare(a.created_at))[0]
    return {
      version: Math.max(0, ...latest.map((item) => item.version_no)),
      status:
        latest.length === 6 && latest.every((item) => item.status === 'published')
          ? 'published'
      ...
      preference_enabled_by_default: boolean(merged.preference_enabled_by_default, true),
      ...
      updated_at: newest?.published_at ?? newest?.created_at ?? new Date(0).toISOString(),
    }
  },
  ...
  async publishSystemSettings(settingIds: string[], reason: string): Promise<void> {
    await adminOpenApiData(
  ...
  async saveAiRunCreditCost(cost: number): Promise<void> {
    ...
    const draft = await adminOpenApiData(
      adminOpenApi.POST('/admin-api/v1/system-settings', {
        body: { section: 'ai', values: { ...values, ai_run_credit_cost: cost } },
      }),
    )
    await adminOpenApiData(
  ...
  async externalKnowledgeSources(): Promise<ExternalKnowledgeSource[]> {
    ...
    return response.items.map((item) => ({
      ...
      secret_configured: item.secret_configured,
      targets: item.targets.map((target) => ({ type: target.type, id: target.id })),
  ...
  async updateExternalKnowledgeSource(
    ...
    await adminOpenApiData(
      adminOpenApi.PATCH('/admin-api/v1/external-knowledge-sources/{source_id}', {
  ...
  async testExternalKnowledgeSource(
    ...
    return adminOpenApiData(
      adminOpenApi.POST('/admin-api/v1/external-knowledge-sources/{source_id}/test', {
        params: { path: { source_id: sourceId } },
      }),
    )
  },

  async syncExternalKnowledgeSource(sourceId: string, reason: string): Promise<void> {
    await adminOpenApiData(
      adminOpenApi.POST('/admin-api/v1/external-knowledge-sources/{source_id}/sync', {
        params: { path: { source_id: sourceId } },
        body: { reason },
      }),
    )
  ...
  async admins(): Promise<AdminAccount[]> {
    const response = await adminOpenApiData(adminOpenApi.GET('/admin-api/v1/admins'))
    return response.items.map((item) => ({
      id: item.id,
      ...
      role: item.permissions.includes('*')
        ? 'super_admin'
        : item.permissions.includes('audit:read') && item.permissions.length === 1
          ? 'auditor'
          : 'operations',
  ...
  async audits(): Promise<AuditLog[]> {
    const response = await adminOpenApiData(
      adminOpenApi.GET('/admin-api/v1/audit-logs', { params: { query: { limit: 500 } } }),
    )
    return response.items.map((item) => ({
      id: item.id,
