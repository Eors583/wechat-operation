...
const props = defineProps<{ modelValue: boolean; blocks: LayoutContentBlock[] }>()
const emit = defineEmits<{
  'update:modelValue': [value: boolean]
  apply: [blocks: LayoutContentBlock[]]
}>()
type Field = { node: Text | HTMLImageElement; value: string; kind: 'text' | 'image' }
  ...
  (open) => {
    ...
    drafts = props.blocks.map((block) => {
      ...
      while (walker.nextNode()) {
        ...
        // Native account/video components must retain their identity metadata.
        if (parent?.closest('[data-profile-card], [data-mpvid], a[href*="action=mpvideo"]'))
          continue
